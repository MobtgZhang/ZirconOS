//! Font Loader — ZirconOS Sun Valley Desktop
//! Manages font resources for the Sun Valley desktop shell, integrating with
//! Font files under src/fonts/ provide a complete
//! font registry for text rendering with WinUI 3 typography.
//!
...
//!
//! The font search path is: src/fonts/{western,cjk}/
...
const FONT_BASE_PATH = "src/fonts";
...
fn setFontPath(dest: *[96]u8, src: []const u8) u8 {
...
fn addFont(name: []const u8, path: []const u8, cat: FontCategory, mono: bool) void {
    ...
    f.path_len = setFontPath(&f.path, path);
    f.is_monospace = mono;
    f.category = cat;
    f.available = true;
    font_count += 1;
