//! Luna Cursor Module
//! Provides smooth cursor state management and the Windows XP-style
//! white arrow cursor bitmap with black outline.
//!
...
pub const SmoothCursorState = struct {
    ...
    pub fn update(self: *SmoothCursorState, raw_x: i32, raw_y: i32, scr_w: i32, scr_h: i32) void {
        ...
        if (dist_sq > 400) {
            ...
            lerp = self.base_lerp + 15;
        } else if (dist_sq < 4) {
            lerp = self.base_lerp - 30;
...
// 0=transparent, 1=fill(white), 2=outline(black), 3=shadow(grey)
pub const luna_cursor_bitmap = [CURSOR_H][CURSOR_W]u2{
