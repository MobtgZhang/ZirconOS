//! ZirconOS Luna Desktop — GDI Compositor Entry Point
//!
//! Implements the Windows XP desktop architecture:
//!   1. Each window renders via GDI to its own off-screen buffer
//!   2. Compositor reads all surfaces and alpha-blends by Z-order
//!   3. Luna effect: gradient titlebars, rounded corners, soft shadows
//!   4. VSync-aligned frame presentation
//!
//! Resources: 本目录下 `resources/`
//! Fonts: `src/fonts/`
//!
...
pub fn main() !void {
    ...
    p(out, "║  Luna GDI Desktop Compositor             ║\n", .{});
    p(out, "╚══════════════════════════════════════════╝\n\n", .{});
    ...
    // ── Phase 3: Initialize GDI Compositor ──
    p(out, "\n--- Phase 3: GDI Compositor Init ---\n", .{});

    shell.initShell();
    compositor.init(SCREEN_W, SCREEN_H);

    p(out, "  Gradients      : {}\n", .{root.isGradientEnabled()});
    p(out, "  Taskbar color  : 0x{X:0>6}\n", .{root.getTaskbarGradientTop()});
    p(out, "  Start button   : 0x{X:0>6}\n", .{root.getStartButtonColor()});
    p(out, "  Shadow size    : {d}\n", .{theme.LunaDefaults.shadow_size});
    p(out, "  Corner radius  : {d}\n", .{theme.LunaDefaults.corner_radius});
    ...
    p(out, "  Window surface    : id={d} (My Computer - gradient+shadow)\n", .{window_surface});
    ...
    p(out, "  Taskbar surface   : id={d} (gradient)\n", .{taskbar_surface});
    ...
    // ── Phase 8: Luna Rendering Pipeline Summary ──
    p(out, "\n--- Luna Rendering Pipeline ---\n", .{});
    p(out, "  ┌─────────────────────────────────────────┐\n", .{});
    p(out, "  │ Application → Off-screen Buffer (GDI)   │\n", .{});
    p(out, "  │         (each window has its own)        │\n", .{});
    p(out, "  ├─────────────────────────────────────────┤\n", .{});
    p(out, "  │ GDI Compositor reads all surfaces        │\n", .{});
    p(out, "  │   ├─ Sort by Z-order                     │\n", .{});
    p(out, "  │   ├─ Gradient titlebar (left→right)      │\n", .{});
    p(out, "  │   ├─ Rounded corners (8px radius)        │\n", .{});
    p(out, "  │   └─ Soft drop shadow (4px)              │\n", .{});
    p(out, "  ├─────────────────────────────────────────┤\n", .{});
    ...
    p(out, "\n--- Resource Integration (ZirconOSLuna/resources) ---\n", .{});
    p(out, "  Start button : resources/start_button.svg\n", .{});
    p(out, "  Cursor       : resources/cursors/luna_arrow.svg\n", .{});
    ...
    p(out, "\n═══ Luna Desktop Ready ═══\n", .{});
    p(out, "GDI compositor running with {d} surfaces, gradients={}, smooth_cursor=true\n", .{
        compositor.getSurfaceCount(),
        root.isGradientEnabled(),
    });
